            ...
            <div class="card">
                ...
                <table id="bootstrap-table" class="table">
                    <thead>
                        ...
                        <th data-field="sale">Sales / Date</th>
                        <th data-field="saledate">Total sales</th>
                    ...
                    <tbody>
                        @if($items)
                            @foreach ($items as $item)
                                <tr id="tax-{{ $item->id }}" class="row-item" data-id="{{ $item->id }}">
                                    <td></td>
                                    <td>{{$item->id}}</td>
                                    <td><a href="{{action('ItemsController@edit', [$item->id])}}">{{$item->name}}</a></td>
                                    <td>{!!number_format($item->getInfo[count($item->getInfo)-1]->salesdate,0)!!}</td>
                                    <td>{!!number_format($item->getInfo[count($item->getInfo)-1]->sales,0)!!}</td>
                                    <td>{{$item->author}}</td>
                                    <td></td>
                                </tr>
                            @endforeach
